...
function getBars(list, tab) {
  const now = new Date()
  const val = list.reduce.bind(list)
  ...
  return [0,3,6,9,12,15,18,21].map(h => ({
    ...
    value: sum(list.filter(e => { const ah=new Date(e.created_at).getHours(); return ah>=h&&ah<h+3 })),
  }))
}
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom:'1rem' }}>
        ...
        {loading ? (
          ...
          <div style={{ padding:'2rem 1.4rem', color:'var(--text-secondary)', fontSize:'.85rem', fontStyle:'italic' }}>No paid enrollments this {tab.toLowerCase()}.</div>
        ) : filtered.map(e => (
