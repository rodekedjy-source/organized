...
export default function Dashboard({ session }) {
  ...
  // FIX #1 — Sign out now redirects properly
      ...
      <div className="db-shell">
        ...
        <div className="db-body">
          ...
          <aside className={`db-sidebar ${sidebarOpen?'open':''}`}>
            ...
            {/* FIX #1 — Sidebar sign out also redirects */}
            <div className="db-nav-item" style={{color:'#c0392b'}} onClick={handleSignOut}>Sign out</div>
          ...
          <main className="db-main">
            {/* FIX #4 — Guard: show loader while workspace is null */}
            {!workspace ? (
              <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'60vh',flexDirection:'column',gap:'1rem'}}>
                <div style={{fontFamily:'Playfair Display,serif',fontSize:'1.2rem',color:'#b5893a'}}>Loading workspace...</div>
              </div>
            ) : (
              <>
...
function Overview({ workspace, toast }) {
  ...
  async function fetchData() {
    ...
    setStats({ revenue, appointments: apptData.length, products: (p.data||[]).length, students: (e.data||[]).length })
...
function Services({ workspace, toast }) {
  ...
  return (
    <div>
      <div className="db-page-head">
        <div><div className="db-page-title">Services</div><div className="db-page-sub">What you offer — appears on your public profile</div></div>
      ...
      <div className="db-card">
        {data.length===0 ? <div style={{padding:'3rem',textAlign:'center',color:'var(--ink-3)',fontSize:'.85rem'}}>No services yet. Add your first service to start receiving bookings.</div>
          : <table className="db-tbl">
...
// ─── AVAILABILITY ─────────────────────────────────────────────────────────────
// FIX #2 — Availability section added (was completely missing)
...
function Availability({ workspace, toast }) {
  ...
  async function fetchData() {
    ...
    if (data && data.length > 0) {
      ...
      // Create default 7-day schedule if none exists
  ...
  if (loading) return <div style={{padding:'3rem',textAlign:'center',color:'var(--ink-3)'}}>Loading availability...</div>

  return (
    <div>
      <div className="db-page-head">
        <div><div className="db-page-title">Availability</div><div className="db-page-sub">Set your weekly working hours</div></div>
        <button className="db-btn db-btn-primary" onClick={save} disabled={saving}>{saving?'Saving...':'Save hours'}</button>
      </div>
      <div className="db-card">
        {schedule.map((day, idx) => (
          <div key={day.day_of_week} style={{display:'flex',alignItems:'center',gap:'1rem',padding:'1rem 1.4rem',borderBottom:'1px solid var(--border)',flexWrap:'wrap'}}>
            <div style={{width:'90px',fontWeight:500,fontSize:'.85rem',color: day.is_open ? 'var(--ink)' : 'var(--ink-3)'}}>
              {DAYS[day.day_of_week]}
            </div>
            <label style={{display:'flex',alignItems:'center',gap:'.5rem',cursor:'pointer',fontSize:'.82rem',color:'var(--ink-3)'}}>
              <input
                type="checkbox"
                checked={day.is_open}
            ...
            {day.is_open && (
              <div style={{display:'flex',alignItems:'center',gap:'.5rem',marginLeft:'auto',flexWrap:'wrap'}}>
                <input
                  ...
                  style={{padding:'.4rem .75rem',border:'1px solid var(--border)',borderRadius:'7px',fontSize:'.82rem',fontFamily:'inherit',color:'var(--ink)',outline:'none'}}
                />
                <span style={{color:'var(--ink-3)',fontSize:'.8rem'}}>to</span>
                <input
                  type="time"
...
// ─── PRODUCTS ────────────────────────────────────────────────────────────────
// FIX #3 — Product image upload added
function Products({ workspace, toast }) {
  ...
  async function add(e) {
    ...
    else { toast(`${form.name} added.`) }

    setForm({name:'',price:'',stock:'',description:''})
    setImageFile(null); setImagePreview(null); setShowForm(false); setUploading(false)
    fetchData()
  }
    ...
    <div>
          ...
          <form onSubmit={add} style={{padding:'1.4rem',display:'flex',flexDirection:'column',gap:'1rem'}}>
            ...
            <div className="db-field">
              ...
              <div
                ...
                style={{border:'2px dashed var(--border)',borderRadius:'10px',padding:'1.5rem',textAlign:'center',cursor:'pointer',background:'#faf9f7',transition:'border .15s'}}
                onMouseEnter={e=>e.currentTarget.style.borderColor='var(--gold)'}
                onMouseLeave={e=>e.currentTarget.style.borderColor='var(--border)'}
              >
                {imagePreview
                  ? <img src={imagePreview} alt="preview" style={{maxHeight:'120px',borderRadius:'8px',objectFit:'cover'}}/>
                  : <div style={{color:'var(--ink-3)',fontSize:'.82rem'}}>Click to upload image<br/><span style={{fontSize:'.72rem'}}>JPG, PNG — max 5MB</span></div>
                }
              </div>
            ...
            <button type="submit" className="db-btn db-btn-primary" style={{width:'100%',justifyContent:'center',padding:'.75rem'}} disabled={uploading}>
      ...
      <div className="db-grid-3">
        {data.length === 0 && !showForm && (
          <div style={{gridColumn:'1/-1',padding:'3rem',textAlign:'center',color:'var(--ink-3)',fontSize:'.85rem',background:'#fff',border:'1px solid var(--border)',borderRadius:'10px'}}>
...
// ─── FORMATIONS ──────────────────────────────────────────────────────────────
// FIX #3 — Formation file upload added
function Formations({ workspace, toast }) {
  ...
  function handleFile(e) {
    ...
    if (!f) return
    if (f.size > 50 * 1024 * 1024) { toast('File too large — max 50MB'); return }
    setFile(f)
  }
  ...
  async function add(e) {
    ...
    if (error) { toast(`Error: ${error.message}`) }
    else { toast(`"${form.title}" created.`) }

    setForm({title:'',price:'',duration_label:'',description:''}); setFile(null); setShowForm(false); setUploading(false)
    fetchData()
  }
      ...
      {showForm && (
        <div className="db-card" style={{marginBottom:'1.25rem'}}>
          ...
          <form onSubmit={add} style={{padding:'1.4rem',display:'flex',flexDirection:'column',gap:'1rem'}}>
            ...
            {/* File upload */}
            <div className="db-field">
              <label>Formation file (PDF, video, ZIP — max 50MB)</label>
              <div
                onClick={()=>fileRef.current.click()}
                style={{border:'2px dashed var(--border)',borderRadius:'10px',padding:'1.25rem',textAlign:'center',cursor:'pointer',background:'#faf9f7'}}
                onMouseEnter={e=>e.currentTarget.style.borderColor='var(--gold)'}
                onMouseLeave={e=>e.currentTarget.style.borderColor='var(--border)'}
              >
                {file
                  ? <div style={{color:'var(--ink)',fontSize:'.82rem'}}>📎 {file.name}</div>
                  : <div style={{color:'var(--ink-3)',fontSize:'.82rem'}}>Click to attach a file<br/><span style={{fontSize:'.72rem'}}>PDF, MP4, ZIP...</span></div>
                }
              </div>
              <input ref={fileRef} type="file" onChange={handleFile} style={{display:'none'}}/>
            </div>
            <button type="submit" className="db-btn db-btn-primary" style={{width:'100%',justifyContent:'center',padding:'.75rem'}} disabled={uploading}>
              {uploading ? 'Uploading...' : 'Save formation'}
            </button>
          </form>
        </div>
      )}
          ...
          : data.map((f,i)=>(
            <div key={f.id} style={{display:'flex',alignItems:'center',gap:'1.25rem',padding:'1.25rem 1.4rem',borderBottom:'1px solid var(--border)'}}>
              ...
              <div style={{flex:1}}>
                ...
                {f.file_url && <a href={f.file_url} target="_blank" rel="noreferrer" style={{fontSize:'.72rem',color:'var(--gold)',textDecoration:'none',marginTop:'.25rem',display:'inline-block'}}>📎 View file</a>}
              </div>
...
.db-grid-3{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-bottom:1.25rem;}
.db-prod-card{background:#fff;border:1px solid var(--border);border-radius:10px;overflow:hidden;}
.db-prod-img{height:140px;background:#f5f3ee;display:flex;align-items:center;justify-content:center;font-size:.72rem;color:var(--ink-3);letter-spacing:.06em;text-transform:uppercase;border-bottom:1px solid var(--border);overflow:hidden;}
...
@media(max-width:600px){.db-stats-row{grid-template-columns:repeat(2,1fr);}.db-grid-3{grid-template-columns:1fr;}.db-main{padding:1.25rem;}.db-page-head{flex-direction:column;align-items:flex-start;}.db-tbl th,.db-tbl td{padding:.6rem .75rem;font-size:.75rem;}}
`
