...
function firstName(workspace, session) {
  if (workspace?.name) return workspace.name.trim().split(' ')[0]
  ...
  return (
    <div>
      ...
      <div className="cal-grid">
        ...
        {cells.map((d,i)=>(
          <div key={i} className={`cal-d${!d?' cal-empty':''}${d===today&&isCurrentMonth?' today':''}${d&&bookedDays.includes(d)?' booked':''}`}>
            {d||''}
          </div>
        ))}
      </div>
...
function WeekChart({ appts }) {
  ...
  const bars = labels.map((_,i)=>{
    const day = new Date(sw); day.setDate(sw.getDate()+i)
    const ds = day.toISOString().split('T')[0]
    return appts.filter(a=>a.scheduled_at?.startsWith(ds)&&a.status==='confirmed')
  ...
  return (
    <div>
      <div className="card-sub" style={{marginBottom:'1rem'}}>Total: {fmtRev(total)}</div>
      <div className="bar-chart">
        {bars.map((v,i)=>(
          <div key={i} className={`bar${i===peakIdx&&v>0?' peak':''}`}
            style={{height:`${Math.max((v/max)*100,4)}%`}}/>
        ))}
      </div>
      <div className="bar-labels">
        {labels.map(l=><div key={l} className="bar-lbl">{l}</div>)}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="rev-overlay" onClick={onClose}>
      <div className="rev-panel" onClick={e=>e.stopPropagation()}>
        ...
        <div className="rev-total">{fmtRev(total)}</div>
...
// ── OVERVIEW ──────────────────────────────────────────────────────────────────
function Overview({ workspace, session, toast, setPage }) {
  ...
  async function fetchData(){
    ...
    setAppts(ad.filter(x=>x.scheduled_at?.startsWith(today)))
  }
  ...
  const cards=[
    {label:'Revenue — '+new Date().toLocaleDateString('en-US',{month:'long'}),value:fmtRev(stats.revenue),delta:'All time',up:true,page:'revenue'},
    {label:'Appointments',value:stats.appointments,delta:stats.pending>0?`&#8595; ${stats.pending} pending confirmation`:'&#10003; All confirmed',up:stats.pending===0,page:'appointments'},
    {label:'Products',value:stats.products,delta:'Listed',up:true,page:'products'},
  ...
  return (
    <div>
      <div className="page-head">
        <div>
          <div className="page-title">{greeting()}, {firstName(workspace,session)}</div>
          <div className="page-sub">{new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'})}{todayCount>0&&` — ${todayCount} appointment${todayCount>1?'s':''} today`}</div>
        </div>
        <div className="head-actions">
          <button className="btn btn-secondary btn-sm" onClick={()=>toast('Booking link copied!')}>Copy booking link</button>
      ...
      <div className="stats-scroll">
        {cards.map((s,i)=>(
          <button key={i} className="stat-card stat-card-btn" onClick={()=>s.page==='revenue'?setShowRevenue(true):setPage(s.page)}>
            ...
            <div className={`stat-delta ${s.up?'delta-up':'delta-down'}`} dangerouslySetInnerHTML={{__html:s.delta}}/>
      ...
      <div className="card">
        ...
        {todayCount===0
          ...
          :<table className="tbl">
            <thead><tr><th>Client</th><th>Service</th><th>Time</th><th>Amount</th></tr></thead>
            <tbody>{appts.map(a=>(
              <tr key={a.id}>
    ...
    <div>
        ...
        {[
          ...
          <div key={i} className="card" style={{marginBottom:0}}>
            <div style={{padding:'.9rem 1.25rem',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <div className="card-sub">{s.l}</div>
              <span className={`badge ${s.cls}`}>{s.v}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {pending.length>0&&(
        <div className="card" style={{marginBottom:'1.25rem'}}>
          ...
          <table className="tbl">
            <thead><tr><th>Client</th><th>Date</th><th>Time</th><th>Amount</th><th>Actions</th></tr></thead>
            <tbody>{pending.map(a=>(
              <tr key={a.id}>
                <td className="tbl-name">{a.client_name}</td>
      ...
      <div className="card">
        ...
        {loading?<div style={{padding:'2rem',color:'var(--ink-3)'}}>Loading...</div>
          :rest.length===0&&pending.length===0?<div style={{padding:'2rem',textAlign:'center',color:'var(--ink-3)',fontSize:'.85rem'}}>No appointments yet.</div>
          :<table className="tbl">
            <thead><tr><th>Client</th><th>Date</th><th>Time</th><th>Amount</th><th>Status</th></tr></thead>
            <tbody>{rest.map(a=>(
              <tr key={a.id}>
                <td className="tbl-name">{a.client_name}</td>
                <td>{new Date(a.scheduled_at).toLocaleDateString()}</td>
...
function Services({ workspace, toast }) {
  ...
  return (
    <div>
      ...
      <div className="card">
        {data.length===0?<div style={{padding:'3rem',textAlign:'center',color:'var(--ink-3)',fontSize:'.85rem'}}>No services yet.</div>
          :<table className="tbl">
  ...
  return (
    <div>
      ...
      <div className="grid-3">
        {data.length===0?<div className="card" style={{gridColumn:'1/-1',padding:'3rem',textAlign:'center',color:'var(--ink-3)',fontSize:'.85rem'}}>No products yet.</div>
          :data.map(p=>(
...
function Clients({ workspace }) {
  ...
  return (
    <div>
      ...
      <div className="card">
        {loading?<div style={{padding:'2rem',color:'var(--ink-3)'}}>Loading...</div>
          :data.length===0?<div style={{padding:'2rem',textAlign:'center',color:'var(--ink-3)',fontSize:'.85rem'}}>No clients yet.</div>
          :<table className="tbl">
...
// ── SETTINGS ──────────────────────────────────────────────────────────────────
function Settings({ workspace, toast, refetch, theme, setTheme }) {
  ...
  async function save(e){
    e.preventDefault();if(!workspace?.id){toast('Error: workspace not loaded.');return}
    setLoading(true);setSaved(false)
    const{error}=await supabase.from('workspaces').update({name:form.name,tagline:form.tagline,location:form.location,instagram:form.instagram,phone:form.phone}).eq('id',workspace.id)
    if(error)toast(`Error: ${error.message}`);else{setSaved(true);toast('Profile saved.');refetch()}
    setLoading(false)
  }
  ...
  return (
    <div>
      <div className="page-head" style={{flexWrap:'wrap',gap:'.75rem'}}>
        <div><div className="page-title">Settings</div><div className="page-sub">Manage your profile</div></div>
        <button className="btn btn-secondary" onClick={publish}>{workspace?.is_published?'Unpublish':'Publish profile'}</button>
      </div>
      <div className="card" style={{marginBottom:'1.25rem'}}>
        <div className="card-head"><div className="card-title">Appearance</div></div>
        <div className="card-body">
          <div style={{fontSize:'.82rem',fontWeight:500,color:'var(--ink)',marginBottom:'.25rem'}}>Theme</div>
          <div style={{fontSize:'.78rem',color:'var(--ink-3)',marginBottom:'1rem'}}>Choose how Organized looks for you.</div>
          <div className="theme-options">
            <div className={`theme-option${!isDark?' selected':''}`} onClick={()=>setTheme('light')}>
              ...
              <div className="theme-label">Light {!isDark&&<div className="theme-check">{I.check}</div>}</div>
            </div>
            <div className={`theme-option${isDark?' selected':''}`} onClick={()=>setTheme('dark')}>
              ...
              <div className="theme-label">Dark {isDark&&<div className="theme-check">{I.check}</div>}</div>
            </div>
          </div>
          <div className="settings-row" style={{marginTop:'1rem'}}>
            <div><div className="settings-row-label">Dark mode</div></div>
      ...
      <div className="card">
        ...
        <form onSubmit={save} className="card-body" style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
          ...
          <button type="submit" className="btn btn-primary" style={{justifyContent:'center',padding:'.75rem'}} disabled={loading}>{loading?'Saving...':saved?'Saved ✓':'Save changes'}</button>
        </form>
      </div>
    </div>
  )
    ...
    <div data-theme={theme} style={{minHeight:'100vh',background:'var(--bg)',transition:'background .3s,color .3s'}}>
      ...
      <div className="topbar">
        <div style={{display:'flex',alignItems:'center',gap:'.85rem'}}>
          ...
          <div className="topbar-logo" style={{cursor:'pointer'}} onClick={()=>{setPage('overview');setView('dashboard');setSidebarOpen(false)}}>
            Organized<span>.</span>
          </div>
        </div>
        ...
        <div className="view-toggle">
          <button className={`vt-btn${view==='dashboard'?' active':''}`} onClick={()=>setView('dashboard')}>
            <span className="vt-label-short">Dash</span>
            <span className="vt-label-full">Dashboard</span>
          </button>
          <button className={`vt-btn${view==='client'?' active':''}`} onClick={()=>setView('client')}>
            <span className="vt-label-short">Client</span>
            <span className="vt-label-full">Client page preview</span>
          </button>
        </div>

        <div className="topbar-right">
          {pendingCount>0&&(
            <button className="notif-btn" onClick={()=>{setPage('appointments');setView('dashboard')}}>
          ...
          <div style={{position:'relative'}}>
            ...
            {showMenu&&(
              <div style={{position:'absolute',right:0,top:'42px',background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'10px',boxShadow:'0 8px 24px rgba(0,0,0,.12)',minWidth:'180px',zIndex:100,overflow:'hidden'}}>
                ...
                <div style={{padding:'.65rem 1rem',fontSize:'.82rem',color:'var(--ink)',cursor:'pointer'}} onClick={()=>{setPage('settings');setShowMenu(false)}}>Settings</div>
      ...
      {view==='dashboard'?(
        <div className="app-shell">
          ...
          <aside className={`sidebar${sidebarOpen?' open':''}`}>
            ...
            {navItems.map(n=>(
              <div key={n.key} className={`nav-item${page===n.key?' active':''}`} onClick={()=>{setPage(n.key);setSidebarOpen(false)}}>
          ...
          <main className="main">
            {page==='overview'    &&<Overview    workspace={workspace} session={session} toast={showToast} setPage={setPage}/>}
            ...
            {page==='settings'    &&<Settings    workspace={workspace} toast={showToast} refetch={fetchWorkspace} theme={theme} setTheme={setTheme}/>}
          </main>
        </div>
      ):(
...
:root{
  ...
  --red:#c0392b;--green:#2e7d52;--blue:#1d4ed8;
...
.notif-btn{position:relative;background:none;border:1px solid var(--border);border-radius:8px;width:34px;height:34px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--ink-3);transition:border-color .15s;}
...
.sidebar-backdrop{position:fixed;inset:0;top:var(--topbar-h);background:rgba(0,0,0,.25);z-index:39;animation:fadeBackdrop .2s ease;}
@keyframes fadeBackdrop{from{opacity:0}to{opacity:1}}
.sidebar{width:var(--sidebar-w);background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;padding:1.25rem 0;flex-shrink:0;position:fixed;top:var(--topbar-h);left:0;bottom:0;z-index:40;transform:translateX(-100%);transition:transform .28s cubic-bezier(.4,0,.2,1),background .3s,border-color .3s;box-shadow:4px 0 24px rgba(0,0,0,.08);}
...
.slug-link:hover{color:var(--gold);text-decoration:underline;cursor:pointer;}
...
.page-head{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:1.75rem;}
...
.stats-scroll{display:flex;gap:1rem;overflow-x:auto;margin-bottom:1.5rem;padding-bottom:.25rem;scrollbar-width:none;-webkit-overflow-scrolling:touch;}
...
.stat-card-btn{text-align:left;cursor:pointer;font-family:inherit;transition:transform .2s cubic-bezier(.34,1.4,.64,1),box-shadow .2s ease,border-color .2s ease;}
.stat-card-btn:hover{transform:translateY(-3px) scale(1.02);box-shadow:0 10px 28px rgba(0,0,0,.1);border-color:var(--gold);}
.stat-card-btn:active{transform:scale(0.96);transition:transform .08s ease;}
...
.bk-btn{background:#111110;color:#fff;border:none;border-radius:7px;padding:.45rem .9rem;font-size:.75rem;cursor:pointer;font-family:inherit;transition:opacity .15s;}
.bk-btn:hover{opacity:.8;}
