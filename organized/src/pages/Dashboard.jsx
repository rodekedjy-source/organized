...
function Services({ workspace, toast, lang='en' }) {
  ...
  const [editImgId,setEditImgId]=useState(null) // service id being re-imaged
  ...
  async function toggle(id,cur){
    await supabase.from('services').update({is_active:!cur}).eq('id',id); fetchData()
  }
  ...
  async function handleEditImg(e,svc){
    ...
    try{
      ...
      await supabase.from('services').update({image_url:url}).eq('id',svc.id)
      toast('Image updated.'); fetchData()
    }catch(err){toast('Upload failed: '+err.message)}
    setEditImgId(null)
  }

  async function removeImg(svc){
    await supabase.from('services').update({image_url:null}).eq('id',svc.id)
    toast('Image removed.'); fetchData()
  }
        ...
        {data.length===0?(
          ...
          <div key={s.id} style={{display:'flex',alignItems:'center',gap:'.85rem',padding:'1rem 1.25rem',borderBottom:'1px solid var(--border)'}}>
            ...
            <div style={{position:'relative',flexShrink:0}}>
              <input ref={editImgId===s.id?editImgRef:null} type="file" accept="image/jpeg,image/png,image/webp" style={{display:'none'}}
                onChange={e=>handleEditImg(e,s)}/>
              {s.image_url?(
    ...
    <div>
      ...
      {photos.length===0
        ...
        : <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:10}}>
            {photos.map((ph,i)=>(
              <div key={ph.id} style={{position:'relative',borderRadius:10,overflow:'hidden',aspectRatio:'1',background:'var(--bg)',cursor:'pointer'}}
                onClick={()=>setLightbox(i)}>
                <img src={ph.url} alt="" style={{width:'100%',height:'100%',objectFit:'cover',display:'block',transition:'transform .3s'}}
