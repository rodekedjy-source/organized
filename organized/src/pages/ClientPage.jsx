...
export default function ClientPage() {
  ...
  // Realtime: blocked dates + availability update live
  useEffect(() => {
    if (!workspace) return

    const channel = supabase
      .channel('client-page-realtime')
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'blocked_dates',
        filter: `workspace_id=eq.${workspace.id}`
      }, (payload) => {
        if (payload.eventType === 'INSERT') {
          setBlockedDates(prev => [...prev, payload.new.blocked_date])
          // If the currently selected date just got blocked, reset selection
          if (selectedDate) {
            const selectedStr = selectedDate.toISOString().split('T')[0]
            if (payload.new.blocked_date === selectedStr) {
        ...
        if (payload.eventType === 'DELETE') {
          setBlockedDates(prev => prev.filter(d => d !== payload.old.blocked_date))
        }
      })
      .on('postgres_changes', {
        ...
        // Refetch availability on any change
      ...
      .on('postgres_changes', {
        ...
        // Refresh booked slots if user is on time selection step
